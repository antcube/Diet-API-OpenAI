...
exports.getCompleteDietPlan = void 0;
const openai_1 = __importDefault(require("openai"));
const zodDietSchemas_1 = require("../schemas/zodDietSchemas");
const nutritionMath_1 = require("../utils/nutritionMath");
const dietAuditor_1 = require("../utils/dietAuditor");
...
const callOpenAI = async (prompt) => {
    const completion = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: 'system',
                content: 'Eres un nutricionista deportivo con enfoque clínico-práctico. Responde exclusivamente JSON válido y prioriza seguridad, adherencia, alimentos reales y coherencia nutricional.'
            },
 ...
 */
const formatFoodPreferences = (foods) => {
    if (!foods || typeof foods !== 'object')
        return 'Sin preferencias específicas.';
    const categoryNames = {
        proteins: 'Proteínas',
    ...
    const categories = Object.entries(foods)
        ...
        .map(([category, list]) => `- ${categoryNames[category] || category.toUpperCase()}: ${list.join(', ')}`);
    return categories.length > 0 ? categories.join('\n') : 'Usa una variedad saludable de alimentos.';
};
/**
 * Construye el prompt dinámico para un día específico
 */
const buildDayPrompt = (day, data, mealNamesStr, userFoodPreferences, isRecipeMode, targetPerMeal, previousIssues) => {
 ...
 */
const generateDay = async (day, data, retries = 3) => {
    ...
    for (let i = 0; i < retries; i++) {
        try {
            const dayPrompt = buildDayPrompt(day, data, mealNamesStr, userFoodPreferences, isRecipeMode, targetPerMeal, previousIssues);
            ...
            const normalizedDay = (0, nutritionMath_1.normalizeDayCalories)(dayJson);
            // Rebalancea el total diario para mantener una brecha pequeña vs objetivo.
            const balancedDay = (0, nutritionMath_1.rebalanceDayToTargetCalories)(normalizedDay, data.calories);
            // Validación estructural estricta con Zod.
            zodDietSchemas_1.DaySchema.parse(balancedDay);
            // Validación de calidad nutricional y restricciones clínicas.
            const qualityIssues = (0, dietAuditor_1.validateDayQuality)(balancedDay, data, mealNames);
            if (qualityIssues.length > 0) {
                const blockingIssues = qualityIssues.filter((issue) => issue.includes('Clínico:') || issue.includes('Estructura:') || issue.includes('Formato:'));
            ...
            if (i === retries - 1)
                throw new Error(`Fallo crítico generando el día ${day}: ${previousIssues}`);
 ...
 */
const generateRecommendations = async (data, retries = 2) => {
    const prompt = `
  ...
  `;
    for (let i = 0; i < retries; i++) {
        try {
            ...
            const validData = zodDietSchemas_1.RecommendationsSchema.parse(parsed);
...
const getCompleteDietPlan = async (data) => {
    try {
...
exports.getCompleteDietPlan = getCompleteDietPlan;
